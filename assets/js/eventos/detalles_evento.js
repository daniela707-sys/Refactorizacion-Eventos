...
function loadBannerConFotosEvento(evento) {
    ...
    // Revisar si el evento tiene fotos
    if (evento.fotos && Array.isArray(evento.fotos) && evento.fotos.length > 0) {

        evento.fotos.forEach((fotoItem, index) => {
            console.log(`📷 Procesando foto ${index + 1} para banner:`, fotoItem);

            try {
                let fotoData;

                // Manejar diferentes formatos de datos
                if (typeof fotoItem.foto === 'string') {
                    fotoData = JSON.parse(fotoItem.foto);
                } else if (typeof fotoItem.foto === 'object') {
                    fotoData = fotoItem.foto;
                } else if (fotoItem.url) {
                    // Si viene directamente una URL
                    imagenes.push(fotoItem.url.startsWith('http') ? fotoItem.url : convertLocalPathToUrl(fotoItem.url));
                    return;
                }

                if (Array.isArray(fotoData)) {
                    fotoData.forEach(img => {
                        if (img.name || img.url) {
                            const imageUrl = img.name ? convertLocalPathToUrl(img.name) :
                                img.url ? (img.url.startsWith('http') ? img.url : convertLocalPathToUrl(img.url)) : null;
                ...
                } else if (fotoData && (fotoData.name || fotoData.url)) {
                    const imageUrl = fotoData.name ? convertLocalPathToUrl(fotoData.name) :
                        fotoData.url ? (fotoData.url.startsWith('http') ? fotoData.url : convertLocalPathToUrl(fotoData.url)) : null;
                ...
                console.error('❌ Error procesando foto para banner:', e, fotoItem);
            }
        });
    }

    // También revisar si hay una imagen principal
    if (evento.imagen && !imagenes.includes(convertLocalPathToUrl(evento.imagen))) {
        imagenes.unshift(convertLocalPathToUrl(evento.imagen));
    }

    // Si hay imágenes del evento, crear banner rotativo con fotos del evento
...
function createEventGallery(evento) {
    ...
    // Campos de imagen del evento
    const camposImagen = ['imagen_principal', 'imagen_secundaria', 'imagen_ubicacion'];
        ...
        if (evento[campo]) {
            try {
                ...
                if (typeof evento[campo] === 'string') {
                    imageData = JSON.parse(evento[campo]);
                } else if (typeof evento[campo] === 'object') {
                    imageData = evento[campo];
                }
                ...
                if (Array.isArray(imageData)) {
                    imageData.forEach(img => {
                        if (img.name) {
                            imagenes.push({
                                url: convertLocalPathToUrl(img.name),
                                alt: evento.nombre || 'Imagen del evento',
                                tipo: campo // Para identificar el tipo de imagen
                            });
                        }
                    });
                } else if (imageData && imageData.name) {
                    // Si es un objeto único
                    imagenes.push({
                        url: convertLocalPathToUrl(imageData.name),
                        alt: evento.nombre || 'Imagen del evento',
                        tipo: campo
                    });
                }
            } catch (e) {
                console.error(`❌ Error procesando ${campo}:`, e, evento[campo]);
            }
        }
    });

    // También revisar si hay fotos adicionales (mantenemos compatibilidad)
    if (evento.fotos && Array.isArray(evento.fotos) && evento.fotos.length > 0) {
        evento.fotos.forEach((fotoItem, index) => {
            try {
                let fotoData;
                ...
                if (typeof fotoItem.foto === 'string') {
                    ...
                    // Si viene directamente una URL
                    imagenes.push({
                        url: fotoItem.url.startsWith('http') ? fotoItem.url : convertLocalPathToUrl(fotoItem.url),
                        alt: evento.nombre || 'Imagen del evento'
                    });
                    return;
                }
                ...
                console.error('❌ Error procesando foto:', e, fotoItem);
            }
        });
    }

    // Eliminar duplicados basados en la URL
    imagenes = imagenes.filter((img, index, self) =>
        index === self.findIndex(i => i.url === img.url)
    );

    // Si no hay suficientes imágenes reales, agregar placeholders relacionados con eventos
    ...
    while (imagenes.length < 4) {
        imagenes.push({
            url: placeholderImages[imagenes.length % placeholderImages.length],
            alt: 'Imagen del evento'
        });
    }

    // Tomar solo las primeras 4 imágenes
            ...
            <div class="gallery-grid">
                ${imagenes.map((img, index) => `
                    <div class="gallery-item" data-index="${index}">
                        <img src="${img.url}" alt="${img.alt}" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=400&q=80'">
                        <div class="gallery-overlay">
                            <div class="gallery-zoom-icon">
                                <svg class="svg-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" preserveAspectRatio="xMidYMid meet">
                                    <g>
                                    <path fill="#FFFFFF" d="M62.242,53.757L51.578,43.093C54.373,38.736,56,33.56,56,28C56,12.536,43.464,0,28,0S0,12.536,0,28
                                    ...
                                    <path fill="#FFFFFF" d="M28,4C14.745,4,4,14.745,4,28s10.745,24,24,24s24-10.745,24-24S41.255,4,28,4z M28,50
                                        C15.85,50,6,40.15,6,28S15.85,6,28,6s22,9.85,22,22S40.15,50,28,50z"/>
                                    <path fill="#FFFFFF" d="M28,11c-0.553,0-1,0.447-1,1s0.447,1,1,1c8.284,0,15,6.716,15,15c0,0.553,0.447,1,1,1s1-0.447,1-1
    ...
    // Insertar después de los detalles del evento
